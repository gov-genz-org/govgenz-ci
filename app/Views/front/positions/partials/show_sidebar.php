<?php

declare(strict_types=1);

helper(['position', 'project']);

/** @var array<string, mixed> $item */
/** @var string $title */
/** @var string $slug */
/** @var list<string> $types */
/** @var array<string, string> $typeLabels */
/** @var list<string> $sectorCodes */
/** @var array<string, string> $sectorFilterPills */
/** @var array<string, string> $sectorOptions */
/** @var string $dateLabel */
/** @var string $readLabel */
/** @var list<array{label: string, href: string, variant: string}> $actionCtas */
/** @var string $positionsListUrl */
/** @var list<array{slug: string, title: string}> $relatedPositions */
/** @var string $shareUrl */
/** @var string $shareQrImageUrl */
/** @var string $shareQrPageUrl */

$shareQrImageUrl = (string) ($shareQrImageUrl ?? '');
$shareQrPageUrl  = (string) ($shareQrPageUrl ?? '');
if ($shareQrImageUrl === '' || $shareQrPageUrl === '') {
    $slug = strtolower(trim($slug, '/'));
    if ($slug !== '') {
        if ($shareQrImageUrl === '') {
            $shareQrImageUrl = position_share_qr_image_url($slug);
        }
        if ($shareQrPageUrl === '') {
            $shareQrPageUrl = position_share_qr_page_url($slug);
        }
    }
}

$shareSocialLinks = project_share_social_links($title, $shareUrl, $shareQrPageUrl);
?>
<aside class="project-sidebar positions-program-show__sidebar" aria-label="<?= esc(lang('Positions.show_sidebar_aria'), 'attr') ?>">

    <div class="widget">
        <div class="widget-title"><?= esc(lang('Positions.show_widget_key_info')) ?></div>
        <?php if ($dateLabel !== '') : ?>
            <div class="widget-row">
                <span><?= esc(lang('Positions.show_widget_published')) ?></span>
                <span><?= esc($dateLabel) ?></span>
            </div>
        <?php endif; ?>
        <?php if ($readLabel !== '') : ?>
            <div class="widget-row">
                <span><?= esc(lang('Positions.show_widget_reading')) ?></span>
                <span><?= esc($readLabel) ?></span>
            </div>
        <?php endif; ?>
        <?php if ($types !== []) : ?>
            <div class="widget-row">
                <span><?= esc(lang('Positions.show_widget_types')) ?></span>
                <span><?= esc(position_type_band_label((string) ($item['types_csv'] ?? ''))) ?></span>
            </div>
        <?php endif; ?>
        <?php if ($sectorCodes !== []) : ?>
            <div class="widget-row">
                <span><?= esc(lang('Positions.show_widget_sectors')) ?></span>
                <span>
                    <?php
                    $sectorLabels = [];
                    foreach (array_slice($sectorCodes, 0, 3) as $code) {
                        $sectorLabels[] = $sectorFilterPills[$code] ?? $sectorOptions[$code] ?? $code;
                    }
                    echo esc(implode(', ', $sectorLabels));
                    ?>
                </span>
            </div>
        <?php endif; ?>
        <p class="projects-program-show__widget-lead positions-program-show__sources-note"><?= esc(lang('Positions.show_sources_note')) ?></p>
    </div>

    <div class="widget">
        <div class="widget-title"><?= esc(lang('Positions.show_widget_actions')) ?></div>
        <p class="projects-program-show__widget-lead"><?= esc(lang('Positions.show_widget_actions_lead')) ?></p>
        <div class="widget-cta">
            <?php foreach ($actionCtas as $cta) :
                $variant = (string) ($cta['variant'] ?? 'ghost');
                $class   = 'projects-program-show__btn positions-program-show__btn--' . $variant;
                ?>
                <a href="<?= esc((string) ($cta['href'] ?? '#'), 'attr') ?>" class="<?= esc($class, 'attr') ?>"><?= esc((string) ($cta['label'] ?? '')) ?></a>
            <?php endforeach; ?>
        </div>
    </div>

    <div class="widget" id="position-share">
        <div class="widget-title"><?= esc(lang('Positions.show_widget_share')) ?></div>
        <div
            class="projects-program-show__share"
            data-project-share
            data-share-url="<?= esc($shareUrl, 'attr') ?>"
            data-share-title="<?= esc($title, 'attr') ?>"
            data-share-text="<?= esc($title . ' — ' . $shareUrl, 'attr') ?>"
            data-share-qr-image="<?= esc($shareQrImageUrl, 'attr') ?>"
            data-share-qr-page="<?= esc($shareQrPageUrl, 'attr') ?>"
            data-share-copied="<?= esc(lang('Positions.show_share_copied'), 'attr') ?>"
            data-share-tiktok-done="<?= esc(lang('Positions.show_share_tiktok_done'), 'attr') ?>"
        >
            <div class="projects-program-show__share-url-head">
                <p class="projects-program-show__widget-lead"><?= esc(lang('Positions.show_widget_share_lead')) ?></p>
                <button
                    type="button"
                    class="projects-program-show__share-icon-btn"
                    data-project-share-copy
                    aria-label="<?= esc(lang('Positions.show_share_copy_aria'), 'attr') ?>"
                    title="<?= esc(lang('Positions.show_share_copy_aria'), 'attr') ?>"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                </button>
            </div>
            <code class="projects-program-show__share-url" data-project-share-url tabindex="0"><?= esc($shareUrl) ?></code>
            <div class="projects-program-show__share-qr-wrap">
                <p class="projects-program-show__share-qr-label"><?= esc(lang('Positions.show_share_qr_label')) ?></p>
                <?= view('front/projects/partials/share_qr_display', [
                    'qrImageUrl'  => $shareQrImageUrl,
                    'qrAlt'       => lang('Positions.show_share_qr_aria'),
                    'imgWidth'    => 140,
                    'imgHeight'   => 140,
                    'qrDataAttr'  => true,
                    'overlayLogo' => false,
                ]) ?>
            </div>
            <div class="projects-program-show__share-social" role="navigation" aria-label="<?= esc(lang('Positions.show_share_social_aria'), 'attr') ?>">
                <a class="projects-program-show__share-social-link" href="<?= esc($shareSocialLinks['facebook']['web'], 'attr') ?>" data-share-href-app="<?= esc($shareSocialLinks['facebook']['app'] ?? '', 'attr') ?>" data-share-href-android="<?= esc($shareSocialLinks['facebook']['android'] ?? '', 'attr') ?>" data-project-share-social="facebook" rel="noopener noreferrer" aria-label="<?= esc(lang('Projects.show_share_social_facebook'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a class="projects-program-show__share-social-link" href="<?= esc($shareSocialLinks['whatsapp']['web'], 'attr') ?>" data-share-href-app="<?= esc($shareSocialLinks['whatsapp']['app'], 'attr') ?>" data-share-href-android="<?= esc($shareSocialLinks['whatsapp']['android'] ?? '', 'attr') ?>" data-project-share-social="whatsapp" rel="noopener noreferrer" aria-label="<?= esc(lang('Projects.show_share_social_whatsapp'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.435 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>
                </a>
                <a class="projects-program-show__share-social-link" href="<?= esc($shareSocialLinks['linkedin']['web'], 'attr') ?>" data-share-href-app="<?= esc($shareSocialLinks['linkedin']['app'] ?? '', 'attr') ?>" data-share-href-android="<?= esc($shareSocialLinks['linkedin']['android'] ?? '', 'attr') ?>" data-project-share-social="linkedin" rel="noopener noreferrer" aria-label="<?= esc(lang('Projects.show_share_social_linkedin'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.062 2.062 0 0 1 2.063-2.065 2.062 2.062 0 0 1 2.063 2.065 2.062 2.062 0 0 1-2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
                <a class="projects-program-show__share-social-link" href="<?= esc($shareSocialLinks['x']['web'], 'attr') ?>" data-share-href-app="<?= esc($shareSocialLinks['x']['app'] ?? '', 'attr') ?>" data-project-share-social="x" rel="noopener noreferrer" aria-label="<?= esc(lang('Projects.show_share_social_x'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <a class="projects-program-show__share-social-link" href="#" data-share-href-app="<?= esc($shareSocialLinks['tiktok']['app'] ?? '', 'attr') ?>" data-project-share-social="tiktok" rel="noopener noreferrer" aria-label="<?= esc(lang('Projects.show_share_social_tiktok'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/></svg>
                </a>
                <a class="projects-program-show__share-social-link" href="<?= esc($shareSocialLinks['email']['web'], 'attr') ?>" data-project-share-social="email" aria-label="<?= esc(lang('Projects.show_share_social_email'), 'attr') ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                </a>
            </div>
        </div>
    </div>

    <?php if ($relatedPositions !== []) : ?>
        <div class="widget">
            <div class="widget-title"><?= esc(lang('Positions.show_widget_see_also')) ?></div>
            <div class="projects-program-show__see-also">
                <?php foreach ($relatedPositions as $rel) : ?>
                    <a href="<?= esc(position_public_url($rel['slug']), 'attr') ?>"><?= esc($rel['title']) ?></a>
                <?php endforeach; ?>
                <a href="<?= esc($positionsListUrl, 'attr') ?>" class="is-muted"><?= esc(lang('Positions.back_to_list')) ?></a>
            </div>
        </div>
    <?php else : ?>
        <div class="widget">
            <div class="widget-title"><?= esc(lang('Positions.show_widget_see_also')) ?></div>
            <div class="projects-program-show__see-also">
                <a href="<?= esc($positionsListUrl, 'attr') ?>"><?= esc(lang('Positions.back_to_list')) ?></a>
            </div>
        </div>
    <?php endif; ?>

</aside>
